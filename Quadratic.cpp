...
void GetSolutionVars(double **p1, double **p2);
// Get the memory for the solution.
...
do {
   ...
   double *px2;
    ...
    // Get the Dynamic Variables for the solution
    GetSolutionVars(&px1, &px2);
  
   // Read the coefficients
   GetCoefficients(coeffs[A_INDEX], coeffs[B_INDEX], coeffs[C_INDEX]);
  
   // Solve the equation
   isReal = SolveQuadratic(coeffs[A_INDEX], coeffs[B_INDEX], coeffs[C_INDEX], px1, px2);

   // Print the results.
   DisplayRoots(*px1, *px2, isReal);
...
// GetSolutionVars dynamically allocates the memory
// for the pointers that are passed in.
void GetSolutionVars(double **p1, double** p2)
{
   double *solutions = new double[ROOTS];
*p1 = solutions + X1_INDEX; // Use pointers in SolveQuadratic
*p2 = solutions + X2_INDEX;
}
